...
onAuthStateChanged(auth, async (user) => {
    if (user && user.emailVerified) {
        ...
        document.getElementById('threadsFeedContainer').innerHTML = '<div style="text-align:center; opacity:0.5; font-size:0.9rem; margin-top:20px;">Login to view threads</div>';
...
// =========================================================================
// GLOBAL THREADS ENGINE (WITH QUOTE REPLIES)
// =========================================================================
let currentReplyTarget = null; // Tracks who you are replying to
let currentReplySnippet = null; // Tracks what they said
...
function listenToGlobalThreads() {
    ...
    threadsUnsubscribe = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            // THE MAGIC: If this message is a reply, build the Quote Block!
            let quoteBlockHTML = "";
            if (data.replyToUser && data.replyToText) {
                quoteBlockHTML = `
                    <div class="thread-reply-quote">
                        <span class="quote-user">Replying to @${data.replyToUser}</span>
                        <span class="quote-text">${data.replyToText}</span>
                    </div>
                `;
            }
            ...
            item.addEventListener('click', () => {
                ...
                currentReplySnippet = data.text; // Store their text
                
                document.getElementById('replyTargetName').innerText = `Replying to @${data.username}`;
                document.getElementById('replyTargetSnippet').innerText = data.text;
...
async function postGlobalThread() {
    ...
    // Grab the stored reply data before we clear the UI
    const replyTarget = currentReplyTarget;
    const replySnippet = currentReplySnippet;

    inputField.value = ""; // Clear for fast UI feedback
    
    // Hide the banner and reset state immediately
    currentReplyTarget = null;
    currentReplySnippet = null;
    document.getElementById('replyPreviewBanner').style.display = 'none';
    ...
    try {
        ...
        // Upload everything to Firebase, including the quoted text!
        ...
        if (results.length === 0) {
            ...
            results.forEach(user => {
                ...
                item.innerHTML = `
                    ...
                    <div class="dm-username" style="margin-left:10px; font-size:1rem; color:var(--greeting-color); font-weight:600;">${user.username}</div>
                `;
                item.addEventListener('click', () => {
...
document.addEventListener('touchmove', (event) => {
    const isInsideScrollableList = event.target.closest('.dm-list, .chat-messages, .drop-search-results, .threads-feed');
    if (!isInsideScrollableList) event.preventDefault(); 
}, { passive: false });


// =========================================================================
// PATCHED: SMART TYPING MODE & KEYBOARD FIX
// =========================================================================
document.querySelectorAll('input').forEach(input => {
    // When the user taps to type, hide the floating UI
    input.addEventListener('focus', () => {
        document.body.classList.add('typing-mode');
    });

    // When the keyboard closes, bring the UI back and fix snap-back
...
// =========================================================================
// THREADS BOX FLIP TOGGLE
// =========================================================================
document.getElementById('toggleThreadsBtn').addEventListener('click', () => {
    // This instantly toggles the CSS 'flipped' class on and off
